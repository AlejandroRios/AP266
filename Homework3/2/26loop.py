...
for n_vort in range(10,100, 10):
    ...
    # Run optimization
    result = minimize (objfun, alpha0, jac = objfungrad, constraints = [con1], method = 'SLSQP',options={'maxiter': 100, 'ftol': 1e-12, 'disp': False})
...
##########################################################################################
    CLL.append(CL)
    CLL_ell.append(CL_ell)
    CDD.append(CD)
    CDD_ell.append(CD_ell)
    nfev_SLSQP.append(result.nfev)
    nfev_SLSQP2.append(result.nfev + result.njev)
    nv.append(n_vort)

    plt.figure(2)
    plt.plot(nv,nfev_SLSQP,'-ob',nv,nfev_SLSQP2,'-ok')
    plt.title("")
    plt.xlabel("num. design variables")
    plt.ylabel("num. func. eval")
    plt.legend(('nfev', 'nfev+njev'),loc='north right')
    plt.grid(True)


    plt.figure(3)
    plt.semilogy(nv,CLL,'-ob',nv,CLL_ell,'-ok')
    plt.title("")
    plt.xlabel("n_vort")
    plt.ylabel("CL")
    plt.grid(True)
    ...
    plt.figure(4)
    plt.plot(nv,CDD,'-ob',nv,CDD_ell,'-ok')
    plt.title("")
    plt.xlabel("n_vort")
    plt.ylabel("CD")
    plt.grid(True)
