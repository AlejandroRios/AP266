...
# Run optimization
result = minimize (objfun, chords, jac = objfungrad, constraints = [con1], method = 'SLSQP',options={'maxiter': 100000, 'ftol': 1e-12, 'disp': False})

# Obtain optimal alpha0, CD and CL
chords = result.x
CD = objfun(chords)
CL = 0.5-confun(chords)
...
plt.title("")
plt.xlabel("b [units]")
plt.ylabel("chords [units]")
plt.grid(True)

plt.figure(3)
plt.semilogy(n_vort,CL,'-ob')
plt.title("")
plt.xlabel("n_vort")
plt.ylabel("CL")
plt.grid(True)
