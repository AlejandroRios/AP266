...
contains
  ...
  subroutine asa_analysis(n_panels, n_cons, Gama, alpha0, chords, &
    ...
    !=====================
    ! YOUR CODE GOES HERE
    real :: Vinfm, rho, CD, Lift,Drag, Dind,mass
    real :: f(2*(n_panels+1)), Loads(n_panels), Xllt(3,n_panels+1), KSmarginLF, marginsLF(2*n_panels)
    integer :: n_nodes, n_beams, n_vort
    !=====================
    n_nodes = n_panels + 1
    n_beams = n_panels
    n_vort = n_panels
    ...
    call tapenade_main(n_vort, Xllt, Gama, alpha0, chords, cl0, cla, Vinf, rhoAir, resllt, Sref, CL, CD, Lift, Dind, Loads)
    call transfer_forces(n_nodes, Xfem, Loads, f)
    call fem_main(n_beams, n_cons, Xfem, r, t, f, d, &
    E, rhoMat, sigmaY, pKS, conIDs, &
    resfem, mass, marginsLF, KSmarginLF)

    margins = (loadFactor*marginsLF) + 1.0 - loadFactor
    KSmargin = (loadFactor*KSmarginLF) + 1.0 - loadFactor
    
    call norm(Vinf, Vinfm)

    Drag = Dind + ((rhoAir*(Vinfm**2.0))/2.0) *Sref*CD0

    FB = (mass + fixedMass)*g*(exp((Endurance*TSFC)/(Lift/Drag))-1.0)

    Weight = FB + (mass+fixedMass)*g

    liftExcess = (Lift/Weight) - 1.0 
